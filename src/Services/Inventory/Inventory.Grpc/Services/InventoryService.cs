...
namespace Inventory.Grpc.Services
{
	public class InventoryService : StockProtoService.StockProtoServiceBase
	{
		...
		{
			_logger.Information(messageTemplate: $"BEGIN Get Stock of ItemNo:{request.ItemNo}");
			...
			_logger.Information(messageTemplate: $"END Get Stock of ItemNo:{request.ItemNo} - Quantity: {result.Quantity}");
			return result;
		}
	}
}
